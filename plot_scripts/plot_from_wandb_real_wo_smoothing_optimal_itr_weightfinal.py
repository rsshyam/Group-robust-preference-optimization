...
def plot_metric_with_error_bands(iteration_index, metric_values, metric_sem, labels, plot_title, subfolder_path, file_name,metric, colors=None, eval_every=192, running_avg_window=None):
    ...
    for avg, sem, label in zip(metric_values, metric_sem, labels):
        ...
        plt.plot(new_iteration_index[:len(avg)], avg, label=legend_label, color=colors[c], linewidth=3)
            
        plt.fill_between(new_iteration_index[:len(avg)], avg - sem, avg + sem, color=colors[c], alpha=0.2)
...
def plot_metric_bars_dpo(metric_config, filters_dicts, subfolder_path, all_avg_metrics_at_iterations, all_sem_metrics_at_iterations):
    ...
    for i, filters_dict in enumerate(filters_dicts):
        ...
        if 'worst_case' not in metric_config['title']:
            plt.xticks(positions, [f"Group {i+1}" for i in range(len(metrics_end_avg))],fontsize=30)
    ...
    if 'worst_case' in metric_config['title']:
        ...
        plt.xticks(all_positions,all_algos,fontsize=50)
    ...
    if 'worst_case' not in metric_config['title']:
        if 'weights' in metric_config["title"]:
            ...
            loc = 'lower right'
        plt.legend(fontsize=40, loc=loc)
        plt.xlabel('Groups',fontsize=55)
    else:
        plt.xlabel('Methods',fontsize=55)
