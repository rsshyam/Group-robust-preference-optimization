    ...
    for avg, sem, label in zip(metric_values, metric_sem, labels):
        ...
        if match:
            ...
            if len(new_iteration_index) != len(avg):
                print(plot_title)
                print(new_iteration_index,avg)
                raise ValueError("Length of iteration_index and avg should be the same.")
        ...
        plt.plot(new_iteration_index, avg, label=label)
        
        plt.fill_between(new_iteration_index, avg - sem, avg + sem, alpha=0.2)
